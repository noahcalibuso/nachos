...
public class Lock {
	...
	if (lockHolder != null) { //if owned
	    waitQueue.waitForAccess(thread); //enqueue
	    KThread.sleep(); // suspend
	}
	else {
	...
	if ((lockHolder = waitQueue.nextThread()) != null)  // checking that the threadqueue is not null
	    lockHolder.ready();
